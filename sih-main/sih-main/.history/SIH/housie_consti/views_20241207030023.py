...
def mark_card_selected(request, room_id):
    try:
        ...
        # Get the case's articles and their article numbers
        case = get_object_or_404(Case, id=current_case['id'])
        case_article_numbers = [art.article_number for art in case.articles.all()]
        
        # Check if the selected article's number matches any in the case
        is_correct = selected_article.article_number in case_article_numbers
        
        if is_correct:
            # Mark card as selected only if correct
            player_id = str(request.user.id)
