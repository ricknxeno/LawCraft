...
def housie_basic(request, room_id):
    ...
    if current_case:
        ...
        current_case['articles'] = case.articles.all()  # Add articles to existing case dict
...
def mark_card_selected(request, room_id):
    try:
        ...
        current_case = get_object_or_404(Case, id=case_id)
        
        # Check if the selected article's number matches any article in the current case
        is_correct = current_case.articles.filter(article_number=selected_article.article_number).exists()
        
        # Record selection time regardless of correctness
        room.record_selection_time(str(request.user.id), article_id, case_id, time_taken)
        
        if is_correct:
            # Handle correct selection (existing code)
            player_id = str(request.user.id)
            ...
            # Record wrong selection
            room.record_wrong_selection(str(request.user.id), article_id, case_id)
        
        return JsonResponse({
            'success': is_correct,
            'message': 'Correct selection!' if is_correct else 'Incorrect selection'
        })
...
def select_article(request, room_id):
    try:
        ...
        case_id = current_case['id']  # Use the current case ID directly
        ...
        # Use the mark_card_selected method which has the correct logic
...
def advance_case(request, room_id):
    try:
        ...
        if game_state.is_active and not room.is_game_completed():
            ...
            # If case didn't change but request was successful
            return JsonResponse({
                'success': True,
                'current_case': room.get_current_case(),
                'time_remaining': game_state.get_time_remaining(),
