...
def housie_basic(request, room_id):
    ...
    if player_id not in room.article_selection:
        ...
        room.save()
        room.assign_articles_to_player(player_id, selected_articles)
...
def room_state(request, room_id):
    ...
    try:
        ...
        # Check if room has 2 players (ready to start)
        game_ready = room.players.count() == 2
        ...
        print(f"Error in room_state: {str(e)}")  # Debug logging
