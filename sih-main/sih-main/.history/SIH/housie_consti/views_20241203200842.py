...
def room_detail(request, room_id):
    ...
    if player_count >= 2:
        return redirect('housie_consti:housie_basic', room_id=room_id)
    ...
    # For development with port number
    if ':' in request.get_host():
        base_url = f"http://{request.get_host()}"
    
    join_url = reverse('housie_consti:join_room', kwargs={'room_id': room_id})
    room_url = f"{base_url}{join_url}"
    ...
    try:
        ...
        if request.user not in room.players.all():
            ...
            if room.players.count() < 2:
                ...
                return redirect('housie_consti:create_room')
