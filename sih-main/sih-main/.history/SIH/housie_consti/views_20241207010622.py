...
def leaderboard(request, room_id):
    ...
    # Get all player points for this room
    players_data = PlayerPoints.objects.filter(room=room).select_related('player').order_by('-points')
    
    # Calculate game duration (in minutes)
    if room.created_at:
        duration = (timezone.now() - room.created_at).total_seconds() / 60
    else:
        duration = 0
    ...
    context = {
        ...
        'total_articles': 15,  # Since each player gets 15 articles
        'achievements': {}  # You can add achievements logic here if needed
    }
