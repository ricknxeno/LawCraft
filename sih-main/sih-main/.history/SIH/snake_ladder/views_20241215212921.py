...
def game_board(request, room_id):
    ...
    if room.winner:
        try:
            # Get final game points for the player
            game_points = room.points.get(str(request.user.id), 0)
            
            # Update overall points first
            overall_points, _ = PlayerOverallPoints.objects.get_or_create(
                player=request.user
            )
            overall_points.total_points = game_points
            overall_points.save()  # This will trigger sync with platform
            
            print(f"[DEBUG] Game ended - Final points for {request.user.username}: {game_points}")
            
        except Exception as e:
            print(f"[DEBUG] Error syncing end-game points: {str(e)}")
