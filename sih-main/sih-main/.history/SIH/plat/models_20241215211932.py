...
class PlayerPlatPoints(models.Model):
    ...
    def update_points(self, points, game_type):
        ...
        print(f"[DEBUG] Updating platform points - Game: {game_type}, Points: {points}")
        
        # Update specific game points
        if game_type == 'SNAKE_LADDER':
            if self.snake_ladder_points != points:  # Only update if different
                self.snake_ladder_points = points
                print(f"[DEBUG] Updated snake ladder points to: {points}")
        ...
        print(f"[DEBUG] After update - flipcard_points: {self.flipcard_points}")
        print(f"[DEBUG] After update - total_points: {self.total_points}")
        
        # Save will handle checkpoint progression
        self.save()
